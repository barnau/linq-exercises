...
namespace LinqExercises.Controllers
{
    public class ProductsController : ApiController
    {
        ...
        {
            var result = _db.Products.Count(p => p.Discontinued == true);
            return Ok(result);
        ...
        public IHttpActionResult GetProductsInCategory(string categoryName)
        {
            ...
            var result = _db.Products.Where(p => p.Category.CategoryName == categoryName);
            return Ok(result);

            // throw new NotImplementedException("Write a query to return all products that fall within the given 
            //categoryName.");
        }
        ...
        public IHttpActionResult GetStockReport()
        {
            ...
            //and the total units in stock for each product labelled as 'TotalStockUnits' where TotalStockUnits is greater than 100.");
        }
